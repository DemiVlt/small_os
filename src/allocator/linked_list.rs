...
impl ListNode {
    ...
    fn alloc_start_addr(&self, size: usize, align: usize) -> Result<usize, ()> {
...
impl LinkedListAllocator {
    ...
    fn find_region(&mut self, size: usize, align: usize) -> Option<(&'static mut ListNode, usize)> {
        ...
        while let Some(ref mut region) = current.next {
            if let Ok(alloc_start) = region.alloc_start_addr(size, align) {
                let next = region.next.take();
                let free_region = current.next.take().unwrap();
                current.next = next;

                return Some((free_region, alloc_start));
            }
